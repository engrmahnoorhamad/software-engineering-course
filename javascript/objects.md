...
Think of it as a container for variables (called properties) and functions (called methods).  **Key-Value Pairs:** Every property has a name (key) and a value. 
...
**Use case**: The standard way to access properties when you know the name and it has no spaces.  **Limitation**: Cannot be used with variables or keys that have spaces. 
